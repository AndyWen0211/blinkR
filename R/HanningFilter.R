#' @export
HanningFilter <- function(x, pupil_col = NULL, window = 11, tofill = TRUE, return_tibble = FALSE,
                          method = c("convolution", "recursive"), sides = 2) {
  method <- match.arg(method)
  if (window %% 2 == 0) stop("Window size must set to be an odd number.")
  weights <- 0.5 - 0.5*cos(2*pi*(0:(window-1))/(window-1))
